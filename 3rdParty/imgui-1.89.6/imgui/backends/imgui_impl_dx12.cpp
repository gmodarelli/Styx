    ...
    for (int n = 0; n < draw_data->CmdListsCount; n++)
    {
        ...
        for (int cmd_i = 0; cmd_i < cmd_list->CmdBuffer.Size; cmd_i++)
        {
            ...
            else
            {
                ...
                texture_handle.ptr = bd->hFontSrvGpuDescHandles[(bd->frameIndex + 1) % bd->numFramesInFlight].ptr;
...
static void ImGui_ImplDX12_CreateFontsTexture()
{
    ...
    // [Solution 3] IDE/msbuild: edit imconfig.h and add '#define ImTextureID ImU64' (prefer solution 2 to create your own config file!)
    // [Solution 4] command-line: add '/D ImTextureID=ImU64' to your cl.exe command-line (this is what we do in the example_win32_direct12/build_win32.bat file)
    static_assert(sizeof(ImTextureID) >= sizeof(bd->hFontSrvGpuDescHandles[0].ptr), "Can't pack descriptor handle into TexID, 32-bit not supported yet.");
    io.Fonts->SetTexID((ImTextureID)bd->hFontSrvGpuDescHandles[0].ptr);
}
                         ...
                         D3D12_CPU_DESCRIPTOR_HANDLE font_srv_cpu_desc_handle2, D3D12_GPU_DESCRIPTOR_HANDLE font_srv_gpu_desc_handle2)
{
    ...
    bd->hFontSrvCpuDescHandles[0] = font_srv_cpu_desc_handle;
    bd->hFontSrvGpuDescHandles[0] = font_srv_gpu_desc_handle;
    bd->hFontSrvCpuDescHandles[1] = font_srv_cpu_desc_handle2;
    bd->hFontSrvGpuDescHandles[1] = font_srv_gpu_desc_handle2;
